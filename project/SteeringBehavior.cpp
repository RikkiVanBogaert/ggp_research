...
void Face::CalculateSteering(SteeringPlugin_Output& steering, const AgentInfo& agentInfo)
{
	...
	/*Elite::Vector2 directionVector{ m_Target - agentInfo.Position };
	directionVector.Normalize();
	const float orientation{ Elite::VectorToOrientation(directionVector)};
	steering.AngularVelocity = orientation - agentInfo.Orientation - M_PI / 2.f;*/
	...
	float turnDirection{};
	constexpr float epsilon{ 0.007f };                                        // - Chosen by iterative testing, (0.01f was og)
...
void FaceAndFlee::CalculateSteering(SteeringPlugin_Output& steering, const AgentInfo& agentInfo)
{
	...
	steering.LinearVelocity = m_Target - agentInfo.Position;
	steering.LinearVelocity.Normalize();
	steering.LinearVelocity *= -agentInfo.MaxLinearSpeed / 2; //only use half speed so we dont lose the enemy
}
